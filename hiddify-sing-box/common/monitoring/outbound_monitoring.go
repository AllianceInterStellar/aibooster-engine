...
const (
	...
	// IP info lookups are only used to show the exit country in the UI, but
	// GetIpInfo walks every provider in turn with a per-provider timeout, and a
	// failure leaves history.IpInfo nil - which makes tester() retry the whole
	// walk for that outbound on every cycle, forever. Back off instead.
	ipInfoBaseBackoff = 5 * time.Minute
	ipInfoMaxBackoff  = 2 * time.Hour
)
...
// ipInfoDue reports whether an IP info lookup should be attempted for this
// outbound. Lookups that keep failing (rate limits, dead providers, a censored
// path) are retried with exponential backoff rather than once per cycle: each
// attempt walks the whole provider list over the user's tunnel, and with every
// outbound doing that concurrently the socket buffers run out - which shows up
// as "write: no buffer space available" and kills live connections.
func (m *OutboundMonitoring) ipInfoDue(out *outboundState) bool {
	out.mu.Lock()
	defer out.mu.Unlock()
	if out.ipInfoFailures == 0 {
		return true
	}
	backoff := ipInfoBaseBackoff << uint(min(out.ipInfoFailures-1, 5))
	if backoff > ipInfoMaxBackoff {
		backoff = ipInfoMaxBackoff
	}
	return time.Since(out.ipInfoFailedAt) >= backoff
}

func (m *OutboundMonitoring) noteIpInfoResult(out *outboundState, err error) {
	out.mu.Lock()
	defer out.mu.Unlock()
	if err == nil {
		out.ipInfoFailures = 0
		out.ipInfoFailedAt = time.Time{}
		return
	}
	out.ipInfoFailures++
	out.ipInfoFailedAt = time.Now()
}
