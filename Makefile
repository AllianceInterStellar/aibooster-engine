.ONESHELL:
PRODUCT_NAME=aibooster-core
BASENAME=$(PRODUCT_NAME)
BINDIR=bin
LIBNAME=$(PRODUCT_NAME)
CLINAME=AiBoosterCli
...
CRONET_GO_VERSION := $(shell cat aibooster-sing-box/.github/CRONET_GO_VERSION)
...
protos:
	...
	protoc --doc_out=./docs  --doc_opt=markdown,aiboosterrpc.md $(shell find v2 -name "*.proto") $(shell find extension -name "*.proto")
...
android: lib_install
	CGO_LDFLAGS="-O2 -g -s -w -Wl,-z,max-page-size=16384" gomobile bind -v -androidapi=21 -javapkg=com.aibooster.core -libname=aibooster-core -tags=$(TAGS) -trimpath -ldflags="$(LDFLAGS)" -target=android -gcflags "all=-N -l" -o $(BINDIR)/$(LIBNAME).aar github.com/sagernet/sing-box/experimental/libbox ./platform/mobile

ios-full: lib_install
	gomobile bind -v  -target ios,iossimulator,tvos,tvossimulator,macos -libname=aibooster-core -tags=$(TAGS),$(IOS_ADD_TAGS) -trimpath -ldflags="$(LDFLAGS)" -o $(BINDIR)/$(PRODUCT_NAME).xcframework github.com/sagernet/sing-box/experimental/libbox ./platform/mobile 
	mv $(BINDIR)/$(PRODUCT_NAME).xcframework $(BINDIR)/$(LIBNAME).xcframework 
	cp AiBoosterCore.podspec $(BINDIR)/$(LIBNAME).xcframework/

ios: lib_install
	gomobile bind -v  -target ios -libname=aibooster-core -tags=$(TAGS),$(IOS_ADD_TAGS) -trimpath -ldflags="$(LDFLAGS)" -o $(BINDIR)/AiBoosterCore.xcframework github.com/sagernet/sing-box/experimental/libbox ./platform/mobile
	cp Info.plist $(BINDIR)/AiBoosterCore.xcframework/
...
windows-amd64: prepare
	...
	$$(go env GOPATH)/bin/rsrc -ico ./assets/aibooster-cli.ico -o ./cmd/bydll/cli.syso ||echo "rsrc error in syso"
