...
func (s *StdNetBind) send(conn *net.UDPConn, pc batchWriter, msgs []ipv6.Message) error {
	...
	if runtime.GOOS == "linux" {
		...
		// WSASendMsg — what WriteMsgUDP becomes on Windows — rejects every one of these
		// sends with WSAEFAULT ("the system detected an invalid pointer address"), so no
		// handshake initiation ever leaves the host and the peer stays down forever.
		// Going around it costs nothing here: the control-message machinery this API
		// exists for is compiled out on Windows (controlSize is 0, setSrcControl and
		// setGSOSize are no-ops, StdNetSupportsStickySockets is false), so msg.OOB is
		// always empty and a plain send puts the identical datagram on the wire. This is
		// also what ClientBind in the sing-box transport has always used, on every
		// platform, without trouble.
		for _, msg := range msgs {
			_, err = conn.WriteToUDPAddrPort(msg.Buffers[0], msg.Addr.(*net.UDPAddr).AddrPort())
			if err != nil {
				break
			}
		}
