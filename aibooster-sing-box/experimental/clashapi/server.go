...
func NewServer(ctx context.Context, logFactory log.ObservableFactory, options option.ClashAPIOptions) (adapter.ClashServer, error) {
	...
	if s.urlTestHistory == nil {
		// The daemon registers its storage by concrete pointer type
		// (service.ContextWithPtr in daemon/instance.go), which the interface
		// lookup above does not find. The outbound monitor resolves it the same
		// way, so without this fallback the API serves a private, always-empty
		// store while the monitor writes to the daemon's: every proxy then shows
		// as timed out in the UI even though traffic is flowing fine.
		if fromDaemon := service.PtrFromContext[urltest.HistoryStorage](ctx); fromDaemon != nil {
			s.urlTestHistory = fromDaemon
		} else {
