	...
	if hopt.EnableClashApi {
		...
		options.Experimental = &option.ExperimentalOptions{
			...
			Monitoring: &option.MonitoringOptions{
				...
				// Left unset this defaults to 10, so a profile with a dozen nodes
				// dials every one of them at once each cycle. Clients run on a
				// single uplink; keep the probe burst well under it.
				Workers: 4,
			},
		}
	}
...
func setRoutingOptions(options *option.Options, hopt *HiddifyOptions) error {
	...
	if hopt.BlockAds {
		rulesets = append(rulesets, option.RuleSet{
			...
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geosite-category-ads-all.srs",
		...
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geosite-malware.srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geosite-phishing.srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geosite-cryptominers.srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geoip-phishing.srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/block/geoip-malware.srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
	...
	if hopt.Region != "other" {
		...
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/country/geoip-" + hopt.Region + ".srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
		rulesets = append(rulesets, option.RuleSet{
			...
			Format: C.RuleSetFormatBinary,
			RemoteOptions: option.RemoteRuleSet{
				URL:            RuleSetBaseURL + "/country/geosite-" + hopt.Region + ".srs",
				UpdateInterval: badoption.Duration(5 * time.Hour * 24),
				DownloadDetour: OutboundSelectTag,
			},
		})
