...
func BuildConfig(ctx context.Context, in *StartRequest) (*option.Options, error) {
	...
	if !in.EnableRawConfig {
		return config.ParseBuildConfig(ctx, static.HiddifyOptions, readOpt)
	}
...
func ChangeHiddifySettings(in *ChangeHiddifySettingsRequest, insert bool) (resp *CoreInfoResponse, retErr error) {
	defer config.DeferPanicToError("changeHiddifySettings", func(err error) {
